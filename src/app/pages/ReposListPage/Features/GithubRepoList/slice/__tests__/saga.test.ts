...
import { githubRepoListSaga, getRepos } from '../saga';
import { RepoErrorType } from '../types';

describe('getRepoList Saga', () => {
  let username: any;
  let repos: any;
  ...
  it('should return error if username is empty', () => {
    ...
    expect(putDescriptor).toEqual(
      put(slice.githubRepoListActions.repoError(RepoErrorType.USERNAME_EMPTY)),
    );
  ...
  it('should dispatch the reposLoaded action if it requests the data successfully', () => {
    ...
    expect(putDescriptor).toEqual(
      put(slice.githubRepoListActions.reposLoaded(repos)),
    );
  });
  ...
  it('should dispatch the user not found error', () => {
    ...
    const putDescriptor = getReposIterator.throw({ response: { status: 404 } })
      .value;
    expect(putDescriptor).toEqual(
      put(slice.githubRepoListActions.repoError(RepoErrorType.USER_NOT_FOUND)),
    );
  });
  it('should dispatch the user has no repo error', () => {
    ...
    expect(putDescriptor).toEqual(
      put(
        slice.githubRepoListActions.repoError(RepoErrorType.USER_HAS_NO_REPO),
      ),
    );
  });
  it('should dispatch the github rate limit error', () => {
    ...
    expect(putDescriptor).toEqual(
      put(
        slice.githubRepoListActions.repoError(RepoErrorType.GITHUB_RATE_LIMIT),
      ),
    );
  });
  ...
  it('should dispatch the response error', () => {
    ...
    expect(putDescriptor).toEqual(
      put(slice.githubRepoListActions.repoError(RepoErrorType.RESPONSE_ERROR)),
    );
  });
});

describe('githubRepoListSaga Saga', () => {
  const githubRepoFormIterator = githubRepoListSaga();
  it('should start task to watch for loadRepos action', () => {
    const takeLatestDescriptor = githubRepoFormIterator.next().value;
    expect(takeLatestDescriptor).toEqual(
      takeLatest(slice.githubRepoListActions.loadRepos.type, getRepos),
    );
  });
});
